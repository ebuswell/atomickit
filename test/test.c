...
static int run_test_fork(void (*fixture)(void (*)()), char *test_name, void (*test)());
...
int (*run_test)(void (*fixture)(void (*)()), char *test_name, void (*test)()) = run_test_fork;
...
static void end_test_nofork(enum test_status status, char *explanation) {
    ...
    if(explanation == NULL) {
	r = test_set_error(errno, "Could not strdup explanation", c_test_res);
	longjmp(test_end_jmp, r == 0 ? 1 : -1);
    }
...
static int test_parse_line(char *line, struct test_result *result) {
	...
	*lineno++ = '\0';
	...
	if(lineno_int > INT_MAX) {
	    return test_set_error(0, "Failure to parse child CHECKPOINT line number: value out of range", result);
	}
	file = strdup(file);
	if(file == NULL) {
	    return test_set_error(errno, "Could not strdup file", result);
	}
	if(result->file != NULL) {
	    free(result->file);
	}
	...
	if(result->explanation == NULL) {
	    return test_set_error(0, "Could not strdup message", result);
	}
    }
    return 0;
}
...
int print_test_results() {
	...
	for(i = 0; i < nresults; i++) {
	    ...
	    r = fprintf(stderr, "%s:%d: %s:%s", file, line, status, test_results->results[i]->test_name);
