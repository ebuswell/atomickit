 ...
 * Atomic Queue
 *
 * Implements an unlimited lock free FIFO queue with reference counted items.
 * Each item should be enqueued in exactly one queue.  The queue will hold a
 * reference to the last dequeued item until the next item is dequeued.
 *
 * This algorithm is a slightly simplified version of Maged M. Michael and
 ...
 */
#ifndef ATOMICKIT_QUEUE_H
#define ATOMICKIT_QUEUE_H 1
 ...
 *
 * @param aqueue a pointer to the queue in which the item is being enqueued.
 ...
 * Dequeues an item.
 *
 * @param aqueue a pointer to the queue from which the item is being dequeued.
 *
 ...
 *
 * @param aqueue a pointer to the queue from which the first item is to be
 * gotten.
 *
 * @returns a pointer to the memory region corresponding to the first item in
 * the queue.
 */
