/** @file atomic-malloc.h
 * Atomic Malloc, Free and Realloc
 *
 * This provides a simple replacement of malloc, free, and realloc
 * that requires no locking. It is optimized for simplicity of code,
 * and may not function well in high-contention situations, though a
 * few simple improvements could remedy that. Currently, everything
 * over 2048 bytes is allocated and freed in page increments directly
 * with mmap. Allocations smaller than that are not freed back to the
 * system, but cached permanently by the allocator. There is zero
 * overhead for all allocations.
 *
 * For simplicity, this is not quite a drop-in replacement for libc's
 * malloc, free, and realloc. Rather, I have offloaded the
 * responsibility for knowing the size of the object to the
 * caller. Except in the case of strings, where a size can easily be
 * obtained with strlen, the size is almost always known by the caller
 * regardless. Although not quite trivial, it would be possible to
 * make these a drop-in replacement for malloc and co. by adding a
